...
function ProductManager() {
  ...
  return <section><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#75965f]">YOUR CATALOGUE</p><h1 className="mt-1 font-serif text-3xl text-[#263327]">My Products</h1><p className="mt-2 max-w-xl text-sm leading-6 text-[#6c776b]">Keep real product information ready for the content that actually needs it.</p></div><Button type="button" onClick={() => open ? reset() : setOpen(true)} aria-expanded={open} className={cn("rounded-xl", open ? "border border-[#b9cdb0] bg-white text-[#476543] hover:bg-[#f2f7ee]" : "bg-[#263327] hover:bg-[#3b4b3b]")}>{open ? <><X className="mr-2 h-4 w-4"/>Close form</> : <><Plus className="mr-2 h-4 w-4"/>Add product</>}</Button></div>
    ...
    {archivedProducts.data?.length ? <section className="mt-8 border-t border-[#e3eadf] pt-6"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#8a7751]">RECOVERY WINDOW</p><h2 className="mt-1 font-serif text-2xl text-[#263327]">Archived products</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#6c776b]">Archived products are not available for new content. Restore one any time before its listed expiry date.</p></div><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{archivedProducts.data.map(product => <article key={product.id} className="flex items-center gap-3 rounded-2xl border border-[#eadfca] bg-[#fffdf8] p-3"><div className="grid h-14 w-14 shrink-0 place-items-center overflow-hidden rounded-xl border border-[#eee3d1] bg-white"><img src={product.imageUrl} alt="" className="h-full w-full object-contain"/></div><div className="min-w-0 flex-1"><p className="truncate font-serif text-lg text-[#3d3a2f]">{product.name}</p><p className="mt-1 text-xs leading-5 text-[#776c59]">Expires {product.archiveExpiresAt ? new Date(product.archiveExpiresAt).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" }) : "soon"}</p></div><Button size="sm" variant="outline" disabled={restore.isPending} onClick={() => restore.mutate({ productId: product.id })} className="shrink-0 rounded-xl border-[#b8cda9] bg-white text-[#466440] hover:bg-[#eef6e9]"><Undo2 className="mr-1.5 h-3.5 w-3.5"/>Restore</Button></article>)}</div></section> : null}
