...
function ProductFlyerPreview({ product }: { product: any }) {
  ...
  return <Popover open={open} onOpenChange={setOpen}><PopoverTrigger asChild><Button type="button" size="sm" variant="outline" onMouseEnter={show} onMouseLeave={hide} onFocus={show} onClick={() => setOpen(value => !value)} className="h-8 rounded-lg border-[#d5e1d0] bg-white px-2.5 text-xs text-[#587052] hover:bg-[#f3f8ef]"><Eye className="mr-1.5 h-3.5 w-3.5"/>Preview</Button></PopoverTrigger><PopoverContent align="start" onMouseEnter={show} onMouseLeave={hide} className="w-64 rounded-2xl border-[#d5e1d0] bg-[#fffefa] p-3 shadow-xl"><div className="overflow-hidden rounded-xl border border-[#e1e8dc] bg-[#263327]"><div className="grid aspect-[4/3] place-items-center bg-[#eff5ea] p-3"><img src={product.imageUrl} alt="" className="h-full w-full object-contain"/></div><div className="px-3 py-3 text-[#f6faed]"><p className="text-[9px] font-bold uppercase tracking-[.14em] text-[#cfe1c3]">EXAMPLE FLYER</p><p className="mt-1 font-serif text-lg leading-tight">{product.name}</p><p className="mt-1 text-xs text-[#d7e4d1]">{product.price ? `₦${product.price}` : "Ask for price"}</p></div></div><p className="mt-2 text-xs leading-5 text-[#687568]">Static preview only. A real flyer is created only after you choose to make one.</p></PopoverContent></Popover>;
}
