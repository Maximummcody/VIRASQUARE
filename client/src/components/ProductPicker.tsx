...
export function ProductPicker({ products, value, onChange, placeholder = "Choose a saved product", label = "Search saved products" }: ProductPickerProps) {
  ...
  return <Popover open={open} onOpenChange={nextOpen => { setOpen(nextOpen); if (!nextOpen) setQuery(""); }}>
    <PopoverTrigger asChild>
      <Button type="button" variant="outline" role="combobox" aria-expanded={open} aria-label={label} className="mt-2 h-11 w-full justify-between rounded-xl border-[#d5e1d0] bg-white px-3 text-left font-normal text-[#334b32] hover:bg-[#f6faf2]">
