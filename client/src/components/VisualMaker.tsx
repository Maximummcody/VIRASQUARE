    ...
    {item.requiresProduct ? <div className="mt-4 rounded-2xl border border-[#cfe0c4] bg-white p-4">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-xl text-xs leading-5 text-[#748174]">{selected ? (stylishGeneration ? `ViraSquare will use ${selected.name}, your saved facts, and your brand details to create an optional AI-enhanced flyer.` : `ViraSquare will use ${selected.name}, your original photo, saved facts, and brand details to build a 4:5 Real Photo Flyer.`) : item.preparationNote || "Choose a real product above first."}</p>
        </div>
        <Button type="button" disabled={!canMakeProductPost || makeVisual.isPending} onClick={() => beginVisual("single_post")} className="w-full shrink-0 rounded-xl bg-[#263327] hover:bg-[#3a4d3b] sm:w-auto">{makeVisual.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <ImagePlus className="mr-2 h-4 w-4" />}{stylishGeneration ? "Generate AI-enhanced flyer" : "Generate Real Photo Flyer"}</Button>
      </div>
      <div className="mt-4 rounded-xl border border-[#dce7d7] bg-[#f8fbf5] p-3"><p className="text-sm font-semibold text-[#405142]">Real Photo Flyer <span className="font-normal text-[#738071]">(default)</span></p><p className="mt-1 text-xs leading-5 text-[#748174]">Keeps your original product photo unchanged. ViraSquare builds the finished 4:5 flyer around your verified product and brand details.</p></div>
      <label className={cn("mt-3 flex cursor-pointer items-start gap-3 rounded-xl border p-3 transition-colors", stylishGeneration ? "border-[#93C5FD] bg-[#EFF6FF]" : "border-[#d9e1df] bg-white hover:border-[#b8c9c2] hover:bg-[#fbfdfb]")}>
        ...
        <span><span className="text-sm font-semibold text-[#334155]">AI-enhanced flyer <span className="font-normal text-[#64748B]">(optional)</span></span><span className="mt-1 block text-xs leading-5 text-[#64748B]">Uses AI to improve the background, lighting, crop, and small visual details. Your product remains recognisable, but the result may differ slightly from the original photo. Instagram will show an AI-content label when you publish this route.</span></span>
      </label>
