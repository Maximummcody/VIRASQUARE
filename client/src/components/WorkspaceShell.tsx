...
function CalendarPanel({ plan, dates, select, makePlan, planning, weeklyPostGoal }: { plan: Item[]; dates: string[]; select: (item: Item) => void; makePlan: () => void; planning: boolean; weeklyPostGoal: number }) {
  ...
  return <section><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#75965f]">YOUR CONTENT PLAN</p><h1 className="mt-1 font-serif text-3xl text-[#263327]">Calendar</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#6c776b]">This is the plan ViraSquare maps around your business and posting rhythm. Open a scheduled post when you are ready to use it.</p></div><Button onClick={makePlan} disabled={planning} className="rounded-xl bg-[#263327] hover:bg-[#3b4b3b]">{planning ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <Sparkles className="mr-2 h-4 w-4"/>}{hasActivePlan ? "Refresh this week" : "Prepare my week"}</Button></div><div className="mt-5 grid overflow-hidden rounded-2xl border border-[#dfe8da] bg-white sm:grid-cols-3"><div className="border-b border-[#e5ece1] px-4 py-3 sm:border-b-0 sm:border-r"><p className="text-[10px] font-bold uppercase tracking-wide text-[#53764b]">PLANNED THIS WEEK</p><p className="mt-1 font-serif text-2xl text-[#334b32]">{plan.length}</p></div><div className="border-b border-[#e5ece1] px-4 py-3 sm:border-b-0 sm:border-r"><p className="text-[10px] font-bold uppercase tracking-wide text-[#806531]">READY TO USE</p><p className="mt-1 font-serif text-2xl text-[#68552e]">{ready}</p></div><div className="px-4 py-3"><p className="text-[10px] font-bold uppercase tracking-wide text-[#55639a]">PREP NEEDED</p><p className="mt-1 font-serif text-2xl text-[#394979]">{productPrep}</p></div></div><div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-7">{dates.map(date => { const item = plan.find(value => value.plannedFor === date); return <article key={date} className={cn("min-h-44 rounded-xl border p-3", item ? "border-[#dbe6d6] bg-[#fffefa]" : hasActivePlan ? "border-[#d4e3cc] bg-[#f4f8f1]" : "border-dashed border-[#dbe3d7] bg-[#fbfcf9]")}><div className="flex items-start justify-between"><div><p className="text-[10px] font-bold uppercase tracking-wide text-[#788676]">{readable(date, { weekday: "short" })}</p><p className="mt-0.5 font-serif text-xl text-[#263327]">{readable(date, { day: "numeric" })}</p></div>{item && <span className="rounded-full bg-[#edf4e9] px-2 py-1 text-[9px] font-bold uppercase tracking-wide text-[#557447]">{item.lifecycleStatus}</span>}</div>{item ? <button onClick={() => select(item)} className="mt-4 text-left"><h2 className="font-serif text-base leading-tight text-[#263327]">{item.title}</h2><p className="mt-2 line-clamp-3 text-xs leading-5 text-[#6d786b]">{item.brief}</p>{item.requiresProduct && <p className="mt-3 flex items-center gap-1 text-[10px] font-bold uppercase tracking-wide text-[#4d793d]"><Package className="h-3 w-3"/>Prepare product</p>}</button> : <div className="mt-5"><p className="text-[10px] font-bold uppercase tracking-[.12em] text-[#5d8152]">{emptyDay.eyebrow}</p><p className="mt-1 text-xs font-semibold leading-5 text-[#40513f]">{emptyDay.title}</p><p className="mt-1 text-[11px] leading-4 text-[#748071]">{emptyDay.detail}</p></div>}</article>; })}</div><div className="mt-4 border-l-2 border-[#b7cfab] pl-3 text-sm leading-6 text-[#6c776b]"><strong className="text-[#334b32]">A simple rule:</strong> Calendar shows ViraSquare’s plan. If you need a different idea for today, choose it from Today rather than building a post from the calendar.</div></section>;
}
