...
function DailyBriefPanel({ current, plan, dates, weeklyPostGoal, onOpenItem, onPrepareWeek, onViewWeek, onNeedProduct }: { current: Item | null; plan: Item[]; dates: string[]; weeklyPostGoal: number; onOpenItem: (item: Item) => void; onPrepareWeek: () => void; onViewWeek: () => void; onNeedProduct: () => void }) {
  ...
  return <section className="space-y-5"><section className="overflow-hidden rounded-[1.5rem] border border-[#d4e3cc] bg-[#fffefa] shadow-sm"><div className="bg-[#263327] p-5 text-[#f7faed] sm:p-6"><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-[#cce1b8]">DAILY BRIEF · {readable(iso(), { weekday: "long", month: "long", day: "numeric" })}</p><h1 className="mt-3 max-w-3xl font-serif text-3xl leading-[1.02] sm:text-4xl">{todayMove.title}</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-[#d6e0d1]">{todayMove.detail}</p></div><Button type="button" onClick={runPrimaryAction} className="w-full rounded-xl bg-[#eaf2ca] text-[#263327] hover:bg-[#f6fadf] sm:w-auto">{todayMove.action === "prepare_product" && <Package className="mr-2 h-4 w-4" />}{todayMove.action === "prepare_week" && <Sparkles className="mr-2 h-4 w-4" />}{todayMove.action === "open_ready" && <CheckCircle2 className="mr-2 h-4 w-4" />}{todayMove.action === "create_today" && <Target className="mr-2 h-4 w-4" />}{todayMove.action === "view_week" && <CalendarDays className="mr-2 h-4 w-4" />}{todayMove.actionLabel}<ChevronRight className="ml-1 h-4 w-4" /></Button></div></div><div className="grid gap-4 p-5 lg:grid-cols-[minmax(0,1fr)_minmax(290px,.8fr)] lg:items-center"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#75965f]">WEEKLY MOMENTUM</p><h2 className="mt-1 font-serif text-2xl leading-tight text-[#263327]">{momentum.title}</h2><p className="mt-2 text-sm leading-6 text-[#697669]">{momentum.detail}</p><div className="mt-4 h-2 overflow-hidden rounded-full bg-[#e7eee2]"><div className="h-full rounded-full bg-[#6f9560] transition-[width] duration-300" style={{ width: `${momentum.percentage}%` }} /></div><p className="mt-2 text-xs font-semibold text-[#587052]">{hasActivePlan ? `${momentum.completedCount} of ${momentum.goal} planned posts completed` : "A focused plan gives your week a clear rhythm."}</p></div><div className="rounded-2xl border border-[#dbe8d5] bg-[#f5faf1] p-4"><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#5d8152]">CHOOSE YOUR PATH</p><p className="mt-2 text-sm leading-6 text-[#405142]">Follow today’s move, or create something different if your business needs another direction.</p><Button type="button" variant="outline" onClick={() => document.getElementById("alternative-ideas")?.scrollIntoView({ behavior: "smooth", block: "start" })} className="mt-4 w-full rounded-xl border-[#b8cdaa] bg-white text-[#45663d] hover:bg-[#edf5e8]">Make something different<ChevronRight className="ml-1 h-4 w-4" /></Button></div></div></section><section><div className="flex items-end justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#75965f]">THIS WEEK AT A GLANCE</p><h2 className="mt-1 font-serif text-2xl text-[#263327]">Your planned rhythm</h2></div><button onClick={onViewWeek} className="shrink-0 text-sm font-semibold text-[#527542]">View calendar <ChevronRight className="inline h-4 w-4" /></button></div><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-7">{dates.map((date, index) => { const item = plan.find(value => value.plannedFor === date); return <article key={date} className={cn("min-h-32 rounded-xl border p-3", index > 1 && "hidden sm:block", item ? "border-[#e2e8de] bg-[#fffefa]" : hasActivePlan ? "border-[#d4e3cc] bg-[#f4f8f1]" : "border-dashed border-[#dce5d8] bg-[#fbfcf9]")}><p className="text-[10px] font-bold uppercase tracking-wider text-[#788676]">{readable(date, { weekday: "short" })}</p><p className="mt-0.5 font-serif text-lg">{readable(date, { day: "numeric" })}</p>{item ? <button onClick={() => onOpenItem(item)} className="mt-3 text-left"><p className="line-clamp-3 text-xs font-medium leading-5 text-[#475347]">{item.title}</p><p className="mt-2 text-[10px] font-bold uppercase text-[#75965f]">{item.lifecycleStatus}</p></button> : <div className="mt-3"><p className="text-[10px] font-bold uppercase tracking-[.1em] text-[#5d8152]">{hasActivePlan ? "REST DAY" : "YOUR PLAN"}</p><p className="mt-1 text-xs font-semibold leading-5 text-[#4b5c49]">{hasActivePlan ? "No post needed today." : "Prepare your week when ready."}</p></div>}</article>; })}</div></section>{hasActivePlan && <p className="rounded-xl border-l-2 border-[#b7cfab] bg-[#f5f9f2] px-4 py-3 text-sm leading-6 text-[#5f715e]">{momentum.completedCount > 0 ? `You have completed ${momentum.completedCount} planned ${momentum.completedCount === 1 ? "post" : "posts"} this week.` : "Your next planned move is ready when you are."}</p>}<div id="alternative-ideas"><AlternativeIdeaGenerator date={iso()} onSelected={onOpenItem} onNeedProduct={onNeedProduct}/></div></section>;
}
