...
import { ViraSquareLogo } from "./ViraSquareLogo";
import { searchSavedProducts } from "@/lib/productPicker";
import {
  Archive,
  CalendarDays,
  CheckCircle2,
...
type Item = {
  ...
  feedbackOutcome?: "not_set" | "conversations" | "orders" | "engagement" | "profile_visits" | "nothing_yet" | "saved_for_later";
...
function WorkspaceEmptyState({ eyebrow, title, detail, actionLabel, onAction, icon: Icon = Package }: { eyebrow: string; title: string; detail: string; actionLabel?: string; onAction?: () => void; icon?: typeof Package }) {
  return <div className="rounded-2xl border border-dashed border-[#BFDBFE] bg-[#F8FBFF] p-6"><Icon className="h-6 w-6 text-[#2563EB]"/><p className="mt-3 text-[10px] font-bold uppercase tracking-[.14em] text-[#2563EB]">{eyebrow}</p><h2 className="mt-2 font-serif text-2xl text-[#0B1220]">{title}</h2><p className="mt-2 max-w-xl text-sm leading-6 text-[#64748B]">{detail}</p>{actionLabel && onAction && <Button type="button" onClick={onAction} className="mt-4 rounded-xl bg-[#0B1220] hover:bg-[#17233A]">{actionLabel}<ChevronRight className="ml-1 h-4 w-4"/></Button>}</div>;
}

function MobileRefreshShell({ onRefresh, children }: { onRefresh: () => Promise<unknown> | unknown; children: React.ReactNode }) {
  const pull = usePullToRefresh(onRefresh);
  const label = pull.refreshing ? "Refreshing" : pull.isReady ? "Release to refresh" : "Pull to refresh";
  return <div className="relative" {...pull.handlers}><div aria-live="polite" className={cn("pointer-events-none -mb-1 flex justify-center overflow-hidden transition-[height,opacity] duration-200 lg:hidden", (pull.distance > 0 || pull.refreshing) ? "h-8 opacity-100" : "h-0 opacity-0")}><span className="inline-flex items-center gap-2 rounded-full bg-[#EFF6FF] px-3 py-1 text-[11px] font-semibold text-[#1D4ED8]"><RefreshCw className={cn("h-3.5 w-3.5", pull.refreshing && "animate-spin")}/>{label}</span></div>{children}</div>;
}
...
function ContentDetail({ item, close }: { item: Item; close: () => void }) {
  ...
  const productNeedsReplacement = Boolean(content.requiresProduct && content.productId && linkedProduct.isError);
  const availableProducts = trpc.virasquare.products.useQuery(undefined, { enabled: Boolean(content.requiresProduct && (!content.productId || productNeedsReplacement)) });
  ...
  const lifecycle = trpc.virasquare.setLifecycle.useMutation({ onSuccess: result => { setContent(result as Item); utils.virasquare.workspace.invalidate(); utils.virasquare.library.invalidate(); const outcome = (result as Item).feedbackOutcome; toast.success(outcome === "saved_for_later" ? "Saved to Drafts. Your post and its saved work are in Library." : ["conversations", "orders", "engagement", "profile_visits"].includes(outcome || "") ? "Thanks. ViraSquare will use that as a gentle cue for future ideas, not as assumed social data." : "Content status updated."); }, onError: error => toast.error(error.message) });
...
function ProductManager() {
  ...
  const archivedProducts = trpc.virasquare.archivedProducts.useQuery();
  ...
  const invalidateProducts = () => { utils.virasquare.products.invalidate(); utils.virasquare.archivedProducts.invalidate(); utils.virasquare.productUsage.invalidate(); utils.virasquare.workspace.invalidate(); };
  ...
  const restore = trpc.virasquare.restoreArchivedProduct.useMutation({ onSuccess: () => { invalidateProducts(); toast.success("Product restored", { description: "It is ready to use in new product posts again." }); }, onError: error => toast.error(error.message) });
  const archive = trpc.virasquare.archiveProduct.useMutation({ onSuccess: ({ archived }) => { invalidateProducts(); if (!archived) return; toast.success("Product moved to Archive", { description: "You can restore it for 30 days before it is permanently removed.", action: { label: "Undo", onClick: () => restore.mutate({ productId: archived.id }) } }); }, onError: error => toast.error(error.message) });
  const emptyArchive = trpc.virasquare.emptyArchivedProducts.useMutation({ onSuccess: ({ deletedCount }) => { setEmptyArchiveOpen(false); invalidateProducts(); toast.success(deletedCount === 1 ? "1 archived product permanently removed" : `${deletedCount} archived products permanently removed`, { description: "Your past posts, flyers, selling packages, downloads, and Library history remain available." }); }, onError: error => toast.error(error.message) });
  const archiveDaysRemaining = (expiresAt: Date | string | null) => expiresAt ? Math.max(0, Math.ceil((new Date(expiresAt).getTime() - Date.now()) / 86_400_000)) : 0;
  ...
  const visibleProducts = useMemo(() => products.data ? searchSavedProducts(products.data, catalogueQuery) : undefined, [products.data, catalogueQuery]);
  const savedProductCount = products.data?.length ?? 0;

  return <MobileRefreshShell onRefresh={() => Promise.all([products.refetch(), archivedProducts.refetch(), usage.refetch()])}><section><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#75965f]">YOUR CATALOGUE</p><h1 className="mt-1 font-serif text-3xl text-[#263327]">My Products</h1><p className="mt-2 max-w-xl text-sm leading-6 text-[#6c776b]">Keep real product information ready for the content that actually needs it.</p></div><Button type="button" onClick={() => open ? reset() : setOpen(true)} aria-expanded={open} className={cn("rounded-xl", open ? "border border-[#b9cdb0] bg-white text-[#476543] hover:bg-[#f2f7ee]" : "bg-[#263327] hover:bg-[#3b4b3b]")}>{open ? <><X className="mr-2 h-4 w-4"/>Close form</> : <><Plus className="mr-2 h-4 w-4"/>Add product</>}</Button></div>
    {!products.data?.length && !open && <div className="mt-5"><WorkspaceEmptyState eyebrow="PRODUCT CONTENT" title="Add your first product when you are ready." detail="Educational content never needs a product. For a product-led post, ViraSquare needs one real image and only the facts you are sure of." actionLabel="Add a product" onAction={() => setOpen(true)}/></div>}
    {savedProductCount > 0 && <div className="mt-5 flex flex-col gap-2 rounded-2xl border border-[#dce6f2] bg-white/90 p-2.5 shadow-sm sm:flex-row sm:items-center sm:justify-between"><div className="relative min-w-0 flex-1"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#2563EB]"/><Input value={catalogueQuery} onChange={event => setCatalogueQuery(event.target.value)} placeholder="Search products by name or price" className="h-10 border-[#bfdbfe] bg-[#f8fbff] pl-9 pr-9 text-sm placeholder:text-[#64748B] focus-visible:ring-[#60A5FA]" aria-label="Search saved products"/>{catalogueQuery && <button type="button" onClick={() => setCatalogueQuery("")} className="absolute right-2 top-1/2 grid h-7 w-7 -translate-y-1/2 place-items-center rounded-lg text-[#2563EB] hover:bg-[#dbeafe] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#60A5FA]" aria-label="Clear product search"><X className="h-3.5 w-3.5"/></button>}</div><div className="flex shrink-0 items-center justify-between gap-2 px-1 text-xs font-semibold text-[#64748B] sm:justify-end"><span>{catalogueQuery ? `${visibleProducts?.length ?? 0} ${visibleProducts?.length === 1 ? "match" : "matches"}` : `${savedProductCount} ${savedProductCount === 1 ? "product" : "products"} saved`}</span>{catalogueQuery && <button type="button" onClick={() => setCatalogueQuery("")} className="hidden text-[#2563EB] hover:text-[#1D4ED8] sm:inline">Clear</button>}</div></div>}
    ...
    {catalogueQuery && !visibleProducts?.length ? <div className="mt-6"><WorkspaceEmptyState eyebrow="NO MATCHES" title="No saved product matches that search." detail="Try a product name, price, or fewer words. Your saved products have not been changed." actionLabel="Clear search" onAction={() => setCatalogueQuery("")}/></div> : <ProductCatalogue products={visibleProducts} usage={usage.data} loading={products.isLoading} onEdit={startEdit} onRemove={setPendingRemoval} onStartProductPost={product => setProductPostStarter({ ...product, key: Date.now() })}/>} 
    {archivedProducts.data?.length ? <section className="mt-8 border-t border-[#e3eadf] pt-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#8a7751]">RECOVERY WINDOW</p><h2 className="mt-1 font-serif text-2xl text-[#263327]">Archived products</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#6c776b]">Archived products are not available for new content. Restore one before its countdown ends. Automatic cleanup checks once daily at 03:00 UTC.</p></div><Button type="button" variant="outline" onClick={() => setEmptyArchiveOpen(true)} className="shrink-0 rounded-xl border-[#d9b6ad] bg-white text-[#914f45] hover:bg-[#fff3f0]"><Trash2 className="mr-2 h-4 w-4"/>Empty Trash</Button></div><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{archivedProducts.data.map(product => { const daysRemaining = archiveDaysRemaining(product.archiveExpiresAt); return <article key={product.id} className="flex items-center gap-3 rounded-2xl border border-[#eadfca] bg-[#fffdf8] p-3"><div className="grid h-14 w-14 shrink-0 place-items-center overflow-hidden rounded-xl border border-[#eee3d1] bg-white"><img src={product.imageUrl} alt="" className="h-full w-full object-contain"/></div><div className="min-w-0 flex-1"><p className="truncate font-serif text-lg text-[#3d3a2f]">{product.name}</p><p className="mt-1 text-xs font-semibold leading-5 text-[#8a6335]">{daysRemaining} {daysRemaining === 1 ? "day" : "days"} remaining</p><p className="text-xs leading-5 text-[#776c59]">Expires {product.archiveExpiresAt ? new Date(product.archiveExpiresAt).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" }) : "soon"}</p></div><Button size="sm" variant="outline" disabled={restore.isPending} onClick={() => restore.mutate({ productId: product.id })} className="shrink-0 rounded-xl border-[#b8cda9] bg-white text-[#466440] hover:bg-[#eef6e9]"><Undo2 className="mr-1.5 h-3.5 w-3.5"/>Restore</Button></article>})}</div></section> : null}
    <AlertDialog open={Boolean(pendingRemoval)} onOpenChange={isOpen => { if (!isOpen) setPendingRemoval(null); }}><AlertDialogContent className="rounded-2xl border-[#e3e9df] bg-[#fffefa]"><AlertDialogHeader><AlertDialogTitle className="font-serif text-xl text-[#263327]">Archive {pendingRemoval?.name}?</AlertDialogTitle><AlertDialogDescription className="leading-6 text-[#687568]">This product will move to Archive and permanently delete after 30 days. Existing posts, flyers, selling packages, downloads, and Library history will remain available. You can restore the product before its expiry date.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel className="rounded-xl" onClick={() => setPendingRemoval(null)}>Cancel</AlertDialogCancel><AlertDialogAction className="rounded-xl bg-[#8b6a38] text-white hover:bg-[#71542e]" disabled={archive.isPending} onClick={() => { if (pendingRemoval) { archive.mutate({ productId: pendingRemoval.id }); setPendingRemoval(null); } }}>Archive product</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
    <AlertDialog open={emptyArchiveOpen} onOpenChange={setEmptyArchiveOpen}><AlertDialogContent className="rounded-2xl border-[#ead6d0] bg-[#fffefa]"><AlertDialogHeader><AlertDialogTitle className="font-serif text-xl text-[#6f342d]">Empty Archive permanently?</AlertDialogTitle><AlertDialogDescription className="leading-6 text-[#687568]">This permanently removes all archived products now. It cannot be undone or restored. Your past posts, flyers, selling packages, downloads, and Library history will stay available.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel className="rounded-xl" disabled={emptyArchive.isPending}>Cancel</AlertDialogCancel><AlertDialogAction className="rounded-xl bg-[#a24f45] text-white hover:bg-[#843c34]" disabled={emptyArchive.isPending} onClick={() => emptyArchive.mutate()}>Empty Trash permanently</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
...
function DailyBriefPanel(props: Parameters<typeof DailyBriefContent>[0]) {
  const today = iso();
  const workspace = trpc.virasquare.workspace.useQuery({ today, weekStart: props.dates[0], weekEnd: props.dates[6] });
  const reminder = trpc.virasquare.preparationReminder.useQuery({ date: today });
  const refreshToday = () => Promise.all([workspace.refetch(), reminder.refetch()]);

  return <MobileRefreshShell onRefresh={refreshToday}><DailyBriefContent {...props}/></MobileRefreshShell>;
}

function DailyBriefContent({ current, plan, dates, weeklyPostGoal, onOpenItem, onPrepareWeek, onViewWeek, onNeedProduct, onOpenAlternative = () => { const target = document.getElementById("alternative-ideas"); target?.scrollIntoView({ behavior: "smooth", block: "center" }); window.setTimeout(() => target?.querySelector<HTMLElement>("button")?.focus({ preventScroll: true }), 350); } }: { current: Item | null; plan: Item[]; dates: string[]; weeklyPostGoal: number; onOpenItem: (item: Item) => void; onPrepareWeek: () => void; onViewWeek: () => void; onNeedProduct: () => void; onOpenAlternative?: () => void }) {
  const hasActivePlan = plan.length >= Math.max(1, weeklyPostGoal);
  ...
  return <section className="space-y-5"><section className="overflow-hidden rounded-[1.5rem] border border-[#DCE6F2] bg-white shadow-sm"><div className="bg-[#0B1220] p-5 text-white sm:p-6"><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-[#93C5FD]">DAILY BRIEF · {readable(today, { weekday: "long", month: "long", day: "numeric" })}</p><h1 className="mt-3 max-w-3xl font-serif text-3xl leading-[1.02] sm:text-4xl">{todayMove.title}</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-[#DBEAFE]">{todayMove.detail}</p></div><Button type="button" onClick={runPrimaryAction} className="group w-full rounded-xl border border-[#93C5FD] bg-white text-[#0B1220] transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#EFF6FF] hover:shadow-lg active:translate-y-0 active:scale-[.98] focus-visible:ring-2 focus-visible:ring-[#60A5FA] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0B1220] sm:w-auto">{todayMove.action === "prepare_product" && <Package className="mr-2 h-4 w-4 text-[#2563EB]" />}{todayMove.action === "prepare_week" && <Sparkles className="mr-2 h-4 w-4 text-[#2563EB]" />}{todayMove.action === "open_ready" && <CheckCircle2 className="mr-2 h-4 w-4 text-[#2563EB]" />}{todayMove.action === "create_today" && <Target className="mr-2 h-4 w-4 text-[#2563EB]" />}{todayMove.action === "view_week" && <CalendarDays className="mr-2 h-4 w-4 text-[#2563EB]" />}{todayMove.actionLabel}<ChevronRight className="ml-1 h-4 w-4 text-[#2563EB] transition-transform duration-200 group-hover:translate-x-0.5" /></Button></div></div><div className="grid gap-4 p-5 lg:grid-cols-[minmax(0,1fr)_minmax(290px,.8fr)] lg:items-center"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#2563EB]">WEEKLY MOMENTUM</p><h2 className="mt-1 font-serif text-2xl leading-tight text-[#0B1220]">{momentum.title}</h2><p className="mt-2 text-sm leading-6 text-[#64748B]">{momentum.detail}</p><div className="mt-4 h-2 overflow-hidden rounded-full bg-[#DBEAFE]"><div className="h-full rounded-full bg-[#2563EB] transition-[width] duration-700 ease-out motion-reduce:transition-none" style={{ width: `${showMomentum ? momentum.percentage : 0}%` }} /></div><p className="mt-2 text-xs font-semibold text-[#1D4ED8]">{hasActivePlan ? `${momentum.completedCount} of ${momentum.goal} planned posts completed` : "A focused plan gives your week a clear rhythm."}</p></div><div className="rounded-2xl border border-[#BFDBFE] bg-white p-4"><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#2563EB]">CHOOSE YOUR PATH</p><p className="mt-2 text-sm leading-6 text-[#334155]">Follow today’s move, or create something different if your business needs another direction.</p><Button type="button" variant="outline" onClick={onOpenAlternative} className="mt-4 w-full rounded-xl border-[#BFDBFE] bg-white text-[#1D4ED8] transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#EFF6FF] hover:shadow-sm active:translate-y-0 active:scale-[.98]">Make something different<ChevronRight className="ml-1 h-4 w-4" /></Button></div></div></section><section><div className="flex items-end justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#2563EB]">THIS WEEK AT A GLANCE</p><h2 className="mt-1 font-serif text-2xl text-[#0B1220]">Your planned rhythm</h2></div><button onClick={onViewWeek} className="shrink-0 rounded-lg px-2 py-1 text-sm font-semibold text-[#2563EB] transition-colors hover:bg-[#EFF6FF] hover:text-[#1D4ED8] focus-visible:ring-2 focus-visible:ring-[#60A5FA]">View calendar <ChevronRight className="inline h-4 w-4" /></button></div><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-7">{dates.map(date => { const item = plan.find(value => value.plannedFor === date); const dateState = getWeeklyDateState(date, today); const isToday = dateState === "today"; const visibleOnPhone = visibleOnMobile.includes(date); const emptyDayDetail = dateState === "past" ? "This day was left clear." : dateState === "future" ? "No post planned." : "No post needed today."; return <article key={date} className={cn("relative min-h-32 rounded-xl border p-3 transition-[transform,box-shadow,border-color,background-color,opacity] duration-200", !visibleOnPhone && "hidden sm:block", item ? "border-[#DCE6F2] bg-white" : hasActivePlan ? "border-[#DCE6F2] bg-[#F8FBFF]" : "border-dashed border-[#DCE6F2] bg-white", dateState === "past" && "opacity-65 saturate-50", dateState === "future" && "bg-white", isToday && "border-[#2563EB] bg-[#EFF6FF] shadow-[0_10px_24px_rgba(37,99,235,0.14)] ring-1 ring-[#BFDBFE] lg:-translate-y-1")}><div className="flex items-center justify-between gap-2"><p className={cn("text-[10px] font-bold uppercase tracking-wider text-[#64748B]", isToday && "text-[#1D4ED8]")}>{isToday ? `Today · ${readable(date, { weekday: "short" })}` : readable(date, { weekday: "short" })}</p>{item && <span className="rounded-full bg-[#F1F5F9] px-2 py-1 text-[9px] font-bold uppercase tracking-wide text-[#475569]">{item.lifecycleStatus}</span>}</div><p className="mt-1 font-serif text-lg text-[#0B1220]">{readable(date, { day: "numeric" })}</p>{item ? <button onClick={() => onOpenItem(item)} className="mt-3 w-full rounded-lg text-left transition-colors hover:bg-[#EFF6FF] focus-visible:ring-2 focus-visible:ring-[#60A5FA]"><p className="line-clamp-3 text-xs font-medium leading-5 text-[#334155]">{item.title}</p><p className={cn("mt-2 text-[10px] font-bold uppercase", dateState === "past" ? "text-[#94A3B8]" : "text-[#2563EB]")}>{item.requiresProduct ? "Prepare product" : "Open post"}</p></button> : <div className="mt-3"><p className="text-[10px] font-bold uppercase tracking-[.1em] text-[#64748B]">{hasActivePlan ? "REST DAY" : "YOUR PLAN"}</p><p className="mt-1 text-xs font-semibold leading-5 text-[#475569]">{hasActivePlan ? emptyDayDetail : "Prepare your week when ready."}</p></div>}</article>; })}</div></section>{hasActivePlan && <p className="rounded-xl border-l-2 border-[#93C5FD] bg-[#EFF6FF] px-4 py-3 text-sm leading-6 text-[#334155]">{momentum.completedCount > 0 ? `You have completed ${momentum.completedCount} planned ${momentum.completedCount === 1 ? "post" : "posts"} this week.` : "Your next planned move is ready when you are."}</p>}<div id="alternative-ideas"><AlternativeIdeaGenerator date={today} onSelected={onOpenItem} onNeedProduct={onNeedProduct}/></div></section>;
}

export function WorkspaceShell({ onEditProfile, initialView = "brief", theme = "default" }: { onEditProfile: () => void; initialView?: View; theme?: "default" | "navy-proof" }) {
  ...
  const resumeItem = current && current.lifecycleStatus !== "posted" && current.lifecycleStatus !== "archived" ? current : plan.find(item => item.lifecycleStatus !== "posted" && item.lifecycleStatus !== "archived") ?? null;
  const refreshCalendar = () => Promise.all([workspace.refetch(), reminder.refetch()]);
  const refreshLibrary = () => Promise.all([workspace.refetch(), library.refetch(), visuals.refetch()]);
  ...
  return <main className={cn("min-h-screen bg-[#f7f7f2] pb-28 text-[#263327] lg:pb-10", theme === "navy-proof" && "navy-proof")}><div className="mx-auto max-w-[1180px] px-3 py-3 sm:px-5 sm:py-4"><header className="rounded-xl border border-white bg-[#fffefa]/95 px-3 py-2.5 shadow-sm sm:px-4"><div className="flex items-center justify-between gap-3"><button onClick={() => navigate("today")} className="flex shrink-0 items-center gap-2.5 text-left" aria-label="ViraSquare home"><ViraSquareLogo small /></button>{nav()}<button onClick={() => navigate("brand")} className="hidden rounded-xl px-3.5 py-2 text-xs font-semibold text-[#667265] hover:bg-[#eef4ea] sm:block">{workspace.data.profile.businessName}</button><button onClick={() => navigate("brand")} className="grid h-8 w-8 place-items-center rounded-full bg-[#e5efdf] text-xs font-bold text-[#537344] sm:hidden">{workspace.data.profile.businessName.slice(0, 1).toUpperCase()}</button></div></header><div className="fixed inset-x-0 bottom-0 z-40 border-t border-[#dfe8da] bg-[#fffefa]/95 shadow-[0_-8px_24px_rgba(38,51,39,0.08)] backdrop-blur lg:hidden"><div className="mx-auto max-w-[680px] px-3 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2">{nav(true)}</div></div>
    ...
    {view !== "brief" && resumeItem && <button type="button" onClick={() => setSelected(resumeItem)} className="mt-3 flex w-full items-center justify-between gap-3 rounded-xl border border-[#BFDBFE] bg-white px-4 py-3 text-left transition-colors hover:bg-[#F8FBFF]"><span className="min-w-0"><span className="block text-[10px] font-bold uppercase tracking-[.14em] text-[#2563EB]">RESUME WORK</span><span className="mt-1 block truncate text-sm font-semibold text-[#0B1220]">{resumeItem.title}</span></span><ChevronRight className="h-4 w-4 shrink-0 text-[#2563EB]"/></button>}
