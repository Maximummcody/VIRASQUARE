...
export function VisualMaker({ item, onOpenProductEducation }: { item: ContentItem; onOpenProductEducation?: (item: ContentItem) => void }) {
  ...
  const captionReview = trpc.virasquare.productPostCaptionReview.useQuery({ deliverableId: deliverable?.id || 0 }, { enabled: Boolean(deliverable?.id && deliverable.type === "single_post" && sellingPackageData) });
  ...
  const saveProductPostCaption = trpc.virasquare.updateProductPostCaption.useMutation({
    onSuccess: async () => {
      setCaptionEditorOpen(false);
      await Promise.all([utils.virasquare.productPostCaptionReview.invalidate({ deliverableId: deliverable?.id || 0 }), utils.virasquare.workspace.invalidate(), utils.virasquare.library.invalidate()]);
      toast.success("Your publish caption has been saved exactly as reviewed.");
    },
  ...
  useEffect(() => {
    if (!captionReview.data || captionEditorOpen) return;
    setCaptionDraft(captionReview.data.caption);
    setCaptionHashtags(captionReview.data.hashtags);
  }, [captionEditorOpen, captionReview.data?.caption, captionReview.data?.hashtags]);
  ...
  const makeVisual = trpc.virasquare.makeVisual.useMutation({
    onSuccess: value => {
      ...
      toast.success(value.slides?.[0]?.sourceMode === "product" ? "Your Real Photo Flyer is ready to review." : "Your AI-enhanced flyer is ready to review.");
    },
    onError: error => {
      setProblem(error.message);
      toast.error(error.message);
  ...
  const captionCharacterCount = `${captionDraft.trimEnd()}${captionHashtags.length ? `\n\n${captionHashtags.join(" ")}` : ""}`.length;
  ...
  return <section className="mt-5 rounded-2xl border border-[#dce8d5] bg-[#f8fbf5] p-4">
    ...
    {item.requiresProduct ? <div className="mt-4 rounded-2xl border border-[#cfe0c4] bg-white p-4">
      <div className="flex flex-col gap-4">
        <div>
          <p className="text-sm font-semibold text-[#344738]">Create your product flyer</p>
      ...
      <div className="mt-4 rounded-xl border border-[#dce7d7] bg-[#f8fbf5] p-3"><p className="text-sm font-semibold text-[#405142]">Real Photo Flyer <span className="font-normal text-[#738071]">(default)</span></p><p className="mt-1 text-xs leading-5 text-[#748174]">A clean, trustworthy post. Your original product photo stays exactly as you saved it while ViraSquare arranges the design, price, and brand details.</p></div>
      <label className={cn("mt-3 flex cursor-pointer items-start gap-3 rounded-xl border p-3 transition-colors", stylishGeneration ? "border-[#93C5FD] bg-[#EFF6FF]" : "border-[#d9e1df] bg-white hover:border-[#b8c9c2] hover:bg-[#fbfdfb]")}>
        <Checkbox checked={stylishGeneration} onCheckedChange={value => setStylishGeneration(value === true)} className="mt-0.5 border-[#64748B] bg-white text-white data-[state=checked]:border-[#2563EB] data-[state=checked]:bg-[#2563EB]" />
        <span><span className="text-sm font-semibold text-[#334155]">AI-enhanced flyer <span className="font-normal text-[#64748B]">(optional)</span></span><span className="mt-1 block text-xs leading-5 text-[#64748B]">Choose this for a more creative, campaign-ready treatment. AI can refine the setting, lighting, framing, and visual polish while keeping your saved product and facts at the centre.</span></span>
      </label>
      <Button type="button" disabled={!canMakeProductPost || makeVisual.isPending} onClick={() => beginVisual("single_post")} className="mt-4 w-full rounded-xl bg-[#263327] hover:bg-[#3a4d3b]">{makeVisual.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <ImagePlus className="mr-2 h-4 w-4" />}Generate my product flyer</Button>
    ...
    {deliverable?.slides?.length > 0 && <div className="mt-5 border-t border-[#dfe9d9] pt-5">
      ...
      {deliverable.type === "single_post" && <section className="mt-4 overflow-hidden rounded-2xl border border-[#cfe0c5] bg-[#f7fbf4]">
        ...
        {sellingPackageData && <div className="grid gap-3 p-4 lg:grid-cols-3">
          <article className="rounded-xl border border-[#dfe8db] bg-white p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#5f8256]">PUBLISH CAPTION</p><h6 className="mt-1 text-sm font-semibold text-[#405142]">Review before posting</h6></div>{captionReview.data && !captionEditorOpen && <Button type="button" size="icon" variant="ghost" title="Copy saved publish caption" onClick={() => copyPackageText(captionReview.data.finalCaption, "Caption")} className="h-8 w-8 text-[#557b45]"><Copy className="h-4 w-4" /></Button>}</div>{captionReview.isLoading ? <div className="mt-3 flex items-center gap-2 text-sm text-[#657563]"><Loader2 className="h-4 w-4 animate-spin" />Loading your saved caption…</div> : captionReview.data ? captionEditorOpen ? <div className="mt-3"><Label htmlFor="publish-caption" className="text-xs font-semibold text-[#405142]">Your exact publish caption</Label><Textarea id="publish-caption" value={captionDraft} onChange={event => setCaptionDraft(event.target.value)} className="mt-2 min-h-40 resize-y" aria-describedby="publish-caption-count" /><p id="publish-caption-count" className={cn("mt-2 text-xs", captionCharacterCount > 2200 ? "text-[#b94b36]" : "text-[#6b786b]")}>{captionCharacterCount.toLocaleString()} / 2,200 characters, including selected hashtags.</p><div className="mt-4 border-t border-[#e8eee4] pt-3"><p className="text-xs font-semibold text-[#405142]">Optional relevant hashtags <span className="font-normal text-[#748174]">(choose up to 5)</span></p><p className="mt-1 text-xs leading-5 text-[#748174]">Suggested only from your saved product and business details. Select any you want, or leave all out.</p><div className="mt-3 flex flex-wrap gap-2">{Array.from(new Set([...captionReview.data.suggestedHashtags, ...captionHashtags])).map(tag => { const selectedTag = captionHashtags.includes(tag); const tagLimitReached = !selectedTag && captionHashtags.length >= 5; return <button key={tag} type="button" aria-pressed={selectedTag} disabled={tagLimitReached} onClick={() => setCaptionHashtags(current => selectedTag ? current.filter(value => value !== tag) : [...current, tag])} className={cn("rounded-full border px-3 py-1.5 text-xs font-semibold transition-colors", selectedTag ? "border-[#2563eb] bg-[#2563eb] text-white" : "border-[#cbd5e1] bg-white text-[#475569] hover:border-[#2563eb] hover:text-[#1d4ed8]", tagLimitReached && "cursor-not-allowed opacity-50")}>{tag}</button>; })}</div></div><div className="mt-4 flex flex-col-reverse gap-2 sm:flex-row sm:items-center sm:justify-end"><Button type="button" variant="ghost" onClick={() => { setCaptionDraft(captionReview.data.caption); setCaptionHashtags(captionReview.data.hashtags); setCaptionEditorOpen(false); }} disabled={saveProductPostCaption.isPending}>Cancel</Button><Button type="button" variant="outline" onClick={() => { setCaptionDraft(captionReview.data.viraSquareCaption); setCaptionHashtags([]); }} disabled={saveProductPostCaption.isPending} className="border-[#b8cdaa] text-[#45663d] hover:bg-[#edf5e8]">Reset to ViraSquare version</Button><Button type="button" onClick={() => saveProductPostCaption.mutate({ deliverableId: deliverable.id, caption: captionDraft, hashtags: captionHashtags })} disabled={saveProductPostCaption.isPending || captionDraft.trim().length < 8 || captionCharacterCount > 2200} className="bg-[#2563eb] hover:bg-[#1d4ed8]">{saveProductPostCaption.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}Save caption</Button></div><p className="mt-3 text-xs leading-5 text-[#748174]">Reset changes this editor only. Save it when you want this exact version used for a future post.</p></div> : <div className="mt-3"><p className="whitespace-pre-wrap text-sm leading-6 text-[#495848]">{captionReview.data.caption}</p>{captionReview.data.hashtags.length > 0 && <p className="mt-3 text-xs font-semibold leading-5 text-[#2563eb]">{captionReview.data.hashtags.join(" ")}</p>}{captionReview.data.canEdit ? <Button type="button" variant="outline" onClick={() => { setCaptionDraft(captionReview.data.caption); setCaptionHashtags(captionReview.data.hashtags); setCaptionEditorOpen(true); }} className="mt-4 w-full border-[#b8cdaa] text-[#45663d] hover:bg-[#edf5e8]">Edit caption</Button> : <p className="mt-3 text-xs leading-5 text-[#748174]">This caption is kept with the post history and cannot change the post already sent to Instagram.</p>}</div> : <p className="mt-3 text-sm leading-6 text-[#657563]">Your matching caption will be ready to review after it is saved.</p>}</article>
    ...
    {educationOpen && <div className="fixed inset-0 z-[60] flex items-end justify-center bg-[#172017]/45 p-0 backdrop-blur-sm sm:items-center sm:p-6"><div role="dialog" aria-modal="true" aria-label="Create an educational carousel about this product" className="max-h-[90dvh] w-full max-w-3xl overflow-y-auto rounded-t-[2rem] bg-[#fffefa] shadow-2xl sm:max-h-[92vh] sm:rounded-3xl"><header className="sticky top-0 flex items-start justify-between gap-4 border-b border-[#e3eadf] bg-[#fffefa]/95 px-5 py-4"><div><p className="eyebrow">SEPARATE PRODUCT EDUCATION</p><h4 className="mt-1 font-serif text-2xl text-[#2d3c2e]">Teach customers about {selected?.name || "this product"}.</h4><p className="mt-2 max-w-2xl text-xs leading-5 text-[#6c786b]">This will be a new educational carousel, separate from the product flyer you just made. ViraSquare will use saved product facts only.</p></div><Button type="button" size="icon" variant="ghost" title="Close educational carousel ideas" onClick={() => setEducationOpen(false)} className="h-10 w-10 shrink-0 rounded-full text-lg">×</Button></header><div className="p-5 pb-[max(2rem,env(safe-area-inset-bottom))]"><Label htmlFor="product-education-topic">What should customers learn? <span className="font-normal text-[#738072]">(optional)</span></Label><Textarea id="product-education-topic" value={educationTopic} onChange={event => setEducationTopic(event.target.value)} placeholder="For example: How to choose an everyday handbag, or what to consider before buying." className="mt-2 min-h-24 resize-none"/><div className="mt-4 flex"><Button type="button" onClick={openEducationIdeas} disabled={suggestProductEducation.isPending} className="w-full rounded-xl bg-[#263327] hover:bg-[#3a4d3b] sm:ml-auto sm:w-auto">{suggestProductEducation.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}Suggest educational angles</Button></div>{educationIdeas.length > 0 && <section className="mt-6 border-t border-[#e5ebe1] pt-5"><p className="eyebrow">CHOOSE AN EDUCATIONAL ANGLE</p><div className="mt-3 grid gap-3 sm:grid-cols-2">{educationIdeas.map((idea, index) => <article key={`${idea.title}-${index}`} className="flex flex-col rounded-2xl border border-[#dfe8db] bg-white p-4"><h5 className="font-serif text-xl leading-tight text-[#334336]">{idea.title}</h5><p className="mt-3 flex-1 text-sm leading-6 text-[#697669]">{idea.brief}</p><Button type="button" variant="outline" disabled={saveProductEducation.isPending} onClick={() => saveProductEducation.mutate({ sourceItemId: item.id, title: idea.title, brief: idea.brief })} className="mt-5 w-full rounded-xl">Choose this educational carousel</Button></article>)}</div></section>}</div></div></div>}
  </section>;
}
