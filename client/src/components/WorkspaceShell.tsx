...
import { getDailyBriefState, getWeeklyMomentum } from "@/lib/dailyBrief";
...
function ProductCatalogue({ products, usage, onEdit, onRemove }: { products: any[] | undefined; usage: Array<{ productId: number; visualCount: number; postedCount: number }> | undefined; onEdit: (product: any) => void; onRemove: (productId: number) => void }) {
  return <>
    <div className="mt-6 space-y-2 sm:hidden">{products?.map(product => {
      const insight = usage?.find(entry => entry.productId === product.id);
      return <article key={product.id} className="flex items-center gap-3 rounded-2xl border border-[#e1e8dc] bg-white p-3 shadow-sm"><button onClick={() => onEdit(product)} className="flex min-w-0 flex-1 items-center gap-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#89aa7b]"><div className="grid h-16 w-16 shrink-0 place-items-center overflow-hidden rounded-xl border border-[#e3eadf] bg-[#f5f8f2]"><img src={product.imageUrl} alt="" className="h-full w-full object-contain"/></div><div className="min-w-0 flex-1"><p className="line-clamp-2 font-serif text-lg leading-tight text-[#263327]">{product.name}</p><p className="mt-1 text-sm font-semibold text-[#61745f]">{product.price ? `₦${product.price}` : "Price on request"}</p><p className="mt-1 text-[10px] font-bold uppercase tracking-wide text-[#71836c]">{insight?.visualCount || 0} visuals · {insight?.postedCount || 0} posted</p></div><ChevronRight className="h-4 w-4 shrink-0 text-[#789176]"/></button><Button onClick={() => onRemove(product.id)} size="icon" variant="ghost" className="h-9 w-9 shrink-0 text-[#a25f55]" aria-label={`Remove ${product.name}`}><Trash2 className="h-4 w-4"/></Button></article>;
    })}</div>
    <div className="mt-6 hidden gap-4 sm:grid sm:grid-cols-2 lg:grid-cols-3">{products?.map(product => {
      const insight = usage?.find(entry => entry.productId === product.id);
      return <article key={product.id} className="overflow-hidden rounded-2xl border border-[#e1e8dc] bg-white"><img src={product.imageUrl} alt={product.name} className="aspect-square w-full object-cover"/><div className="p-4"><div className="flex justify-between gap-2"><div><h2 className="font-serif text-xl text-[#263327]">{product.name}</h2><p className="mt-1 text-sm text-[#6f7c6e]">{product.price ? `₦${product.price}` : "Price on request"}</p></div><Button onClick={() => onRemove(product.id)} size="icon" variant="ghost" className="h-8 w-8 text-[#a25f55]" aria-label={`Remove ${product.name}`}><Trash2 className="h-4 w-4"/></Button></div><p className="mt-3 line-clamp-2 text-sm leading-5 text-[#738072]">{product.details || "Add product facts to strengthen future product content."}</p><Button onClick={() => onEdit(product)} variant="outline" className="mt-4 h-8 rounded-lg text-xs">Edit product details</Button><div className="mt-4 flex gap-2 text-[10px] font-bold uppercase tracking-wide"><span className="rounded-full bg-[#eef6e9] px-2 py-1 text-[#557445]">{insight?.visualCount || 0} visuals</span><span className="rounded-full bg-[#eef0ff] px-2 py-1 text-[#506099]">{insight?.postedCount || 0} posted</span></div></div></article>;
    })}</div>
  </>;
}
...
function ProductManager() {
  ...
  return <section><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#75965f]">YOUR CATALOGUE</p><h1 className="mt-1 font-serif text-3xl text-[#263327]">My Products</h1><p className="mt-2 max-w-xl text-sm leading-6 text-[#6c776b]">Keep real product information ready for the content that actually needs it.</p></div><Button onClick={() => open ? reset() : setOpen(true)} className="rounded-xl bg-[#263327] hover:bg-[#3b4b3b]"><Plus className="mr-2 h-4 w-4"/>{open ? "Close" : "Add product"}</Button></div>
    ...
    <ProductCatalogue products={products.data} usage={usage.data} onEdit={startEdit} onRemove={productId => remove.mutate({ productId })}/>
  </section>;
}
