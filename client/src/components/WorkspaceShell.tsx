...
function ProductManager() {
  ...
  const remove = trpc.virasquare.deleteProduct.useMutation({ onSuccess: () => { invalidateProducts(); toast.success("Product deleted", { description: "It has been removed from My Products." }); }, onError: error => toast.error(error.message) });
  ...
  return <section><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#75965f]">YOUR CATALOGUE</p><h1 className="mt-1 font-serif text-3xl text-[#263327]">My Products</h1><p className="mt-2 max-w-xl text-sm leading-6 text-[#6c776b]">Keep real product information ready for the content that actually needs it.</p></div><Button type="button" onClick={() => open ? reset() : setOpen(true)} aria-expanded={open} className={cn("rounded-xl", open ? "border border-[#b9cdb0] bg-white text-[#476543] hover:bg-[#f2f7ee]" : "bg-[#263327] hover:bg-[#3b4b3b]")}>{open ? <><X className="mr-2 h-4 w-4"/>Close form</> : <><Plus className="mr-2 h-4 w-4"/>Add product</>}</Button></div>
    ...
    <AlertDialog open={Boolean(pendingRemoval)} onOpenChange={isOpen => { if (!isOpen) setPendingRemoval(null); }}><AlertDialogContent className="rounded-2xl border-[#e3e9df] bg-[#fffefa]"><AlertDialogHeader><AlertDialogTitle className="font-serif text-xl text-[#263327]">Delete {pendingRemoval?.name}?</AlertDialogTitle><AlertDialogDescription className="leading-6 text-[#687568]">This will remove the saved product from My Products. Your existing posts and flyers will remain available, but you will not be able to use this product in new product posts unless you add it again.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel className="rounded-xl" onClick={() => setPendingRemoval(null)}>Cancel</AlertDialogCancel><AlertDialogAction className="rounded-xl bg-[#a24f45] text-white hover:bg-[#843c34]" disabled={remove.isPending} onClick={() => { if (pendingRemoval) { remove.mutate({ productId: pendingRemoval.id }); setPendingRemoval(null); } }}>Delete product</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
