...
function CalendarPanel({ plan, dates, select, makePlan, planning }: { plan: Item[]; dates: string[]; select: (item: Item) => void; makePlan: () => void; planning: boolean }) {
  const ready = plan.filter(item => Boolean(item.caption)).length;
  const productPrep = plan.filter(item => item.requiresProduct).length;
  return <section><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#75965f]">YOUR CONTENT PLAN</p><h1 className="mt-1 font-serif text-3xl text-[#263327]">Calendar</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#6c776b]">This is the plan ViraSquare maps around your business and posting rhythm. Open a scheduled post when you are ready to use it.</p></div><Button onClick={makePlan} disabled={planning} className="rounded-xl bg-[#263327] hover:bg-[#3b4b3b]">{planning ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}{plan.length > 1 ? "Refresh this week" : "Prepare my week"}</Button></div><div className="mt-6 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-[#edf4e9] p-4"><p className="text-[10px] font-bold uppercase tracking-wide text-[#53764b]">PLANNED THIS WEEK</p><p className="mt-2 font-serif text-3xl text-[#334b32]">{plan.length}</p></div><div className="rounded-2xl bg-[#fff2d6] p-4"><p className="text-[10px] font-bold uppercase tracking-wide text-[#806531]">READY TO USE</p><p className="mt-2 font-serif text-3xl text-[#68552e]">{ready}</p></div><div className="rounded-2xl bg-[#edf0ff] p-4"><p className="text-[10px] font-bold uppercase tracking-wide text-[#55639a]">PREP NEEDED</p><p className="mt-2 font-serif text-3xl text-[#394979]">{productPrep}</p></div></div><div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-7">{dates.map(date => { const item = plan.find(value => value.plannedFor === date); return <article key={date} className={cn("min-h-52 rounded-2xl border p-4", item ? "border-[#dbe6d6] bg-[#fffefa]" : "border-dashed border-[#dbe3d7] bg-[#fbfcf9]")}><div className="flex items-start justify-between"><div><p className="text-[10px] font-bold uppercase tracking-wide text-[#788676]">{readable(date, { weekday: "short" })}</p><p className="mt-1 font-serif text-2xl text-[#263327]">{readable(date, { day: "numeric" })}</p></div>{item && <span className="rounded-full bg-[#edf4e9] px-2 py-1 text-[9px] font-bold uppercase tracking-wide text-[#557447]">{item.lifecycleStatus}</span>}</div>{item ? <button onClick={() => select(item)} className="mt-6 text-left"><h2 className="font-serif text-lg leading-tight text-[#263327]">{item.title}</h2><p className="mt-3 line-clamp-3 text-xs leading-5 text-[#6d786b]">{item.brief}</p>{item.requiresProduct && <p className="mt-4 flex items-center gap-1 text-[10px] font-bold uppercase tracking-wide text-[#4d793d]"><Package className="h-3 w-3" />Prepare product</p>}</button> : <p className="mt-8 text-xs leading-5 text-[#9aa49a]">ViraSquare has left this day open.</p>}</article>; })}</div><div className="mt-5 rounded-2xl border border-[#e1e8dc] bg-white p-4 text-sm leading-6 text-[#6c776b]"><strong className="text-[#334b32]">A simple rule:</strong> Calendar shows ViraSquare’s plan. If you need a different idea for today, choose it from Today rather than building a post from the calendar.</div></section>;
}
